 ...
 */
export class HistorianAgent extends BaseAgent {
  public readonly name = 'Historian';
  public readonly description = 'Provides historical context, facts, and educational information about locations and points of interest';

  private readonly historyKeywords = [
  ...
  getIntentMatch(input: string, context: ConversationContext): IntentMatch | null {
    ...
    return {
      confidence,
      priority: confidence > 0.7 ? AgentPriority.HIGH : AgentPriority.MEDIUM,
   ...
   */
  private determineInformationDepth(context: ConversationContext): string {
