...
exports.runPremiumAgentPrompt = runPremiumAgentPrompt;
exports.runStandardAgentPrompt = runStandardAgentPrompt;
exports.runOfflineAgentPrompt = runOfflineAgentPrompt;
exports.runSmartAgentPrompt = runSmartAgentPrompt;
 ...
 */
async function runPremiumAgentPrompt(agentName, systemPrompt, userInput, context) {
    try {
        ...
        const result = await (0, openaiRealtimeEngine_1.testOpenAIRealtime)(fullPrompt);
        if (!result.success) {
 ...
 */
function buildAgentPrompt(agentName, systemPrompt, userInput, context) {
    const sections = [
...
function buildContextSection(context) {
    ...
    if (context.tourState) {
        ...
        if (currentMarkerId)
            contextParts.push(`Current marker: ${currentMarkerId}`);
        if (nextMarkerId)
            contextParts.push(`Next marker: ${nextMarkerId}`);
    }
    // User preferences
 ...
 */
function buildHistorySection(context, maxMessages = 5) {
 ...
 */
async function runStandardAgentPrompt(agentName, systemPrompt, userInput, context) {
 ...
 */
async function runOfflineAgentPrompt(agentName, systemPrompt, userInput, context) {
 ...
 */
async function runSmartAgentPrompt(agentName, systemPrompt, userInput, context) {
    const brain = (0, config_1.getBrainForEnvironment)();
