...
class AgentRouter {
    constructor() {
        ...
        this.agents = [
            new navigatorAgent_1.NavigatorAgent(),
            new historianAgent_1.HistorianAgent(),
            new experienceAgent_1.ExperienceAgent(),
            new memoryAgent_1.MemoryAgent()
        ];
        console.log(`Agent Router: Initialized with ${this.agents.length} agents:`, this.agents.map(a => a.name).join(', '));
    }
    /**
     * Route user input to the most appropriate agent
     */
    async routeToAgent(input, context) {
        try {
     ...
     */
    applySelectionRules(candidates, input, context) {
     ...
     */
    buildReasoningText(selected, alternatives, input) {
        const reasons = [];
     ...
     */
    async testAllAgents(input, context) {
        const results = [];
        for (const agent of this.agents) {
            ...
            results.push({
                ...
                priority: intentMatch?.priority || types_1.AgentPriority.LOW
            });
        }
 ...
 */
async function routeToAgent(input, context) {
